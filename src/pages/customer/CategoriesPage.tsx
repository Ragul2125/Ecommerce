...
import type { Category, SuperCategory } from "@/utils/mockData"
...
export function CategoriesPage() {
  ...
  const [activeSuperId, setActiveSuperId] = useState<string>("s1")
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] bg-muted/10 overflow-hidden">
      {/* 1. Minimal Header */}
              ...
              >
                ...
                <div className="pt-10 space-y-6">
                   ...
                   <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {['Zara Studio', 'H&M Move', 'Nike Lab', 'Gucci Archive'].map((brand, _i) => (
                        <button key={brand} className="h-16 border border-border/10 rounded-[2rem] flex items-center justify-center px-4 bg-muted/5 font-black text-[9px] uppercase tracking-widest text-muted-foreground/30 hover:text-primary hover:bg-primary/5 hover:border-primary/20 transition-all duration-500">
                          {brand}
                        </button>
                      ))}
                   </div>
