...
export function OrderConfirmationPage() {
  ...
  // Mock order data
  const orderNumber = "AQ-" + Math.floor(100000 + Math.random() * 900000)
  
  return (
    <div className="flex flex-col items-center justify-center min-h-[90vh] px-6 gap-12 pb-40">
       ...
       <div className="relative">
          ...
          >
             <div className="h-full w-full rounded-full bg-primary flex items-center justify-center shadow-glow">
                <Check className="h-12 w-12 md:h-16 md:w-16 text-white stroke-[4]" />
             </div>
          </motion.div>
       ...
       <motion.div 
         ...
         animate={{ opacity: 1, scale: 1 }}
         transition={{ delay: 0.5 }}
         className="w-full max-w-sm rounded-[3.5rem] bg-zinc-900 text-white p-10 space-y-10 shadow-glow relative overflow-hidden"
       >
          ...
          <div className="space-y-6 relative z-10">
             <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.2em] opacity-40">
                ...
                <span className="text-white opacity-100">{orderNumber}</span>
             </div>
             <div className="flex justify-between items-end">
                <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Total Valuation</span>
                <span className="text-4xl font-black text-white tracking-tighter">₹1,25,000</span>
             </div>
          </div>
          ...
          <div className="pt-8 border-t border-white/10 space-y-3 relative z-10">
             ...
             <div className="space-y-1">
                <p className="text-sm font-black uppercase tracking-widest text-primary">Resident Associate</p>
                <p className="text-xs font-medium text-white/60 leading-relaxed">123 Premium Way, Architecture District<br/>San Francisco, CA 94103</p>
             </div>
          </div>
       </motion.div>
       ...
       >
          <Button asChild className="h-18 rounded-full bg-white text-black font-black text-[10px] uppercase tracking-widest shadow-2xl hover:bg-zinc-100 active:scale-[0.98] transition-all">
             <Link to="/categories">Continue Curation</Link>
          </Button>
          <Button asChild variant="ghost" className="h-18 rounded-full font-black text-[10px] uppercase tracking-widest text-muted-foreground hover:text-white hover:bg-white/5">
