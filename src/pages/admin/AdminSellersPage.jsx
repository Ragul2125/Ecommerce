...
function AdminSellersPage() {
  ...
  const filteredSellers = sellers.filter(
    (seller) => seller.name.toLowerCase().includes(searchQuery.toLowerCase()) || seller.company.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  />
    ...
    /* Sellers List */
      ...
      <div className="glass rounded-[2rem] overflow-hidden shadow-premium border border-border/40 bg-card">
        {filteredSellers.length === 0 ? <div className="p-16 text-center space-y-4">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border/30">
                {filteredSellers.map((seller) => <tr key={seller.id} className="hover:bg-muted/10 transition-colors">
                    <td className="p-6">
                    ...
                    <td className="p-6">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wide ${seller.status === "Active" ? "bg-green-500/10 text-green-600" : "bg-orange-500/10 text-orange-600"}`}>
                        {seller.status}
                      </span>
                    </td>
    ...
    variant="ghost"
    size="sm"
    className={`rounded-full text-xs font-bold ${seller.status === "Active" ? "text-orange-600 hover:text-orange-700 hover:bg-orange-100 dark:hover:bg-orange-900/30" : "text-green-600 hover:text-green-700 hover:bg-green-100 dark:hover:bg-green-900/30"}`}
  >
                        {seller.status === "Active" ? <><ShieldBan className="mr-2 h-3 w-3" /> Suspend</> : <><ShieldCheck className="mr-2 h-3 w-3" /> Activate</>}
                      </Button>
                    </td>
                  </tr>)}
              </tbody>
