...
const initialProducts = [
  ...
  { id: "PRD-103", name: "Smart Home Hub", seller: "Tech Haven", price: 149.5, category: "Electronics", status: "Disabled" }
];
function AdminProductsPage() {
  ...
  const filteredProducts = products.filter(
    (p) => p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.seller.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        {filteredProducts.length === 0 ? <div className="p-16 text-center space-y-4">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border/30">
                {filteredProducts.map((product) => <tr key={product.id} className="hover:bg-muted/10 transition-colors">
                    <td className="p-6">
                    ...
                    <td className="p-6">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wide ${product.status === "Active" ? "bg-green-500/10 text-green-600" : "bg-orange-500/10 text-orange-600"}`}>
                        {product.status}
                      </span>
                    </td>
