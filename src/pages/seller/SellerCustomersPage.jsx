        ...
        {filteredCustomers.length === 0 ? <div className="p-16 text-center space-y-4 bg-background/50">
            ...
            <table className="w-full text-left border-collapse bg-background/50">
              ...
              <tbody className="divide-y divide-border/30">
                {filteredCustomers.map((c) => <tr key={c.id} className="hover:bg-muted/10 transition-colors">
                    <td className="p-6">
                    ...
                    <td className="p-6">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wide ${c.status === "Active" ? "bg-green-500/10 text-green-600" : "bg-muted text-muted-foreground"}`}>
                        {c.status}
                      </span>
                    </td>
                  </tr>)}
